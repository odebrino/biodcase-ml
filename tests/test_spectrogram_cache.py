...
def test_spectrogram_cache_falls_back_when_torchaudio_import_is_broken(tmp_path, monkeypatch):
    real_import = builtins.__import__

    def broken_torchaudio_import(name, *args, **kwargs):
        if name == "torchaudio" or name.startswith("torchaudio."):
            raise OSError("libtorch_cuda.so: cannot open shared object file")
        return real_import(name, *args, **kwargs)

    monkeypatch.setattr(builtins, "__import__", broken_torchaudio_import)
    audio_path = tmp_path / "event.wav"
    wavfile.write(audio_path, 250, torch.zeros(500).numpy().astype("float32"))
    audio_cfg = {
        "sample_rate": 250,
    ...
    tensor = cached_event_tensor(row, audio_cfg, img_size=64, cache_cfg={"root": tmp_path / "cache"})
